...
const computeTotals = (items: CartItem[], shippingCost: number = defaultShippingCost) => {
    ...
    return { subtotal, shipping, finalTotal, totalItems };
};
        ...
        if (typeof window !== "undefined") {
            try {
                ...
                if (stored) {
                    ...
                    const getter = get();
                    const totals = computeTotals(items, getter.shipping);
